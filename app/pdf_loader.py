...
def load_pdf_from_url(url: str) -> list[Document]:
    ...
    # Step 3: Load with LangChain loader
...
def add_to_chroma_db(chunks: list[Document]):
    ...
    db = chroma_client.get_or_create_collection(name="my_collection", embedding_function=get_embedding_function())
    ...
    if len(new_chunks):
        ...
        new_chunk_contents = [chunk.page_content for chunk in new_chunks]
        db.upsert(documents=new_chunk_contents, ids=new_chunk_ids)
    else:
